...
export function getGlassBlurPx(mode?: string | null) {
  return mode === "liquid" ? glassConfig.liquidBlurPx : glassConfig.blurPx;
}

// 为模糊核留出四倍半径，防止调整模糊值后产生透明边缘。
// 至少保留 1px，让关闭模糊时的 Canvas 边缘采样仍有有效尺寸。
export function getGlassBlurPadding(mode?: string | null) {
  return Math.max(1, Math.ceil(getGlassBlurPx(mode) * 4));
}

export const glassBlurPadding = getGlassBlurPadding();

// 根布局在首屏 HTML 中输出变量；CSS 与 Canvas/SVG 共用上面的配置。
export const glassCssVariables: CSSProperties & Record<`--${string}`, string> =
  {
    "--app-glass-standard-blur": `${glassConfig.blurPx}px`,
    "--app-glass-liquid-blur": `${glassConfig.liquidBlurPx}px`,
    ...
    "--glass-standard-blur-padding": `${glassBlurPadding}px`,
    "--glass-liquid-blur-padding": `${getGlassBlurPadding("liquid")}px`,
  };
